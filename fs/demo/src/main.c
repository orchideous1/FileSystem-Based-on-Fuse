...
static void* demo_mount(struct fuse_conn_info * conn_info){
    ...
    int ret = 0;
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &ret);
    /* 填充super信息 */
    super.sz_io = ret;
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &ret);
    super.sz_disk = ret;
...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    ddriver_read(super.driver_fd, blk, super.sz_io);
    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    memcpy(&curr, blk, sizeof(struct demo_dentry));
    /* TODO: 填充filename */
    strcpy(filename, curr.fname);
    printf("%s", filename);
